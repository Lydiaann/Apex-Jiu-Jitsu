...
//   }
// })();
// form.js — CLEAN, STABLE, MOBILE-SAFE MODAL LOGIC
(() => {
  const MAX_TRIES = 60;
  const POLL_INTERVAL = 100;
  ...
  const poll = setInterval(() => {
    ...
    if (modal && form) {
      clearInterval(poll);
      initModalLogic(modal, form);
    }

    if (tries >= MAX_TRIES) {
      clearInterval(poll);
      console.warn("form.js: timed out waiting for modal/form.");
    }
  }, POLL_INTERVAL);

  function initModalLogic(modal, form) {
    const closeBtn = modal.querySelector(".close");
    ...
    document.addEventListener("click", (e) => {
      const trigger = e.target.closest(".open-form");
      if (!trigger) return;
    ...
    console.info("form.js: initialized successfully");
  }
})();
